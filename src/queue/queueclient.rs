...
use crate::{
    data::{EntryID, Layout, SlotMetaTile, SlotTile, SummaryTile, TileID},
    deferred_data::DeferredDataSource,
    http::fetch::ProfResponse,
    queue::work::{Data, Work},
    timestamp::Interval,
};

use crate::http::fetch::fetch;

use crate::http::schema::{FetchRequest, FetchTilesRequest};

pub struct HTTPQueueDataSource {
    pub url: Url,
    pub client: Client,
    pub queue: Arc<Mutex<Vec<Work>>>,
    pub info: Option<Layout>,
        ...
        fetch(request, move |result: Result<ProfResponse, String>| {
            ...
            match work {
                ...
                Work::FetchInfo(_data) => {
                    work =
                        Work::FetchInfo(Data::Ready(serde_json::from_str::<Layout>(text).unwrap()));
                }
            }
